...
void ChatDialog::drawStaticElements() {
    ...
    tft.setCursor(15, 10);
    tft.print("DEEPSEEK");
    tft.pushImage(tft.width() - 110, 5, 100, 20, ELECROW_LOGO);    // Draw Elecrow company Logo

    // Display current status icon
    if (currentMicSpkStatus == 1) {
        tft.pushImage(tft.width()/2 - 20, 8, 20, 20, MIC_LOGO);
    } else if (currentMicSpkStatus == 2) {
        tft.pushImage(tft.width()/2 - 20, 8, 20, 20, SPK_LOGO);
    }
}
