...
/**
 * @brief Show the latest received message on the LCD.
 *
 * The receiver prints each payload to the serial monitor and mirrors it on
 * the display so students can verify radio reception without watching only
 * the computer log.
 *
...
// Radio pipe address shared with the transmitter sketch.
const byte address[6] = "00001";

/**
 * @brief Initialize the LCD and configure the nRF24L01 receiver.
 *
 * Arduino calls this function once after reset. It enables the wireless
 * module path, starts the radio on the configured SPI pins, opens the reading
 * pipe, and switches the radio into listening mode.
 *
 * @param None.
 * @return None.
 */
void setup() {
  ...
  /*---------------------------------------------------------------
   * Start the radio as a receiver
   * Channel, data rate, and address must match the transmitter or no
...
// Counts received packets so the LCD text visibly changes over time.
int i=0;

/**
 * @brief Read available radio payloads and display them.
 *
 * Arduino calls this function repeatedly after setup() finishes. When a
 * payload arrives, the receiver copies it from the radio FIFO, prints it to
 * serial, and draws it on the LCD with a running count.
 *
 * @param None.
 * @return None.
 */
